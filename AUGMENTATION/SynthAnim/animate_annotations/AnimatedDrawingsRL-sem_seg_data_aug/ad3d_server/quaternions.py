...
class Quaternions:
    ...
    def rotate_between_vectors(cls, v1: Vectors, v2: Vectors) -> Quaternions:
        ...
        # if vectors are parallel, perturb slightly to avoid nans
        if norm(c) == 0:
            v1 += np.random.rand(*v1.shape) * 0.0001
            v1 /= norm(v1)
            c = np.cross(v1, v2)
        ...
        w = np.sqrt(norm(v1)**2 * norm(v2)**2) + d
        x = c[:, 0]
        y = c[:, 1]
        z = c[:, 2]
        ...
        return Quaternions(q)

    @classmethod
    def from_angle_axis(cls, angles_rads: npt.NDArray[np.float32], axes: Vectors) -> Quaternions:
        axes.norm()

        if len(angles_rads.shape) == 1:
            angles_rads = np.expand_dims(angles_rads, axis=0)

        ss = np.sin(angles_rads / 2.0)
        cs = np.cos(angles_rads / 2.0)
