...
from animated_drawings.model.bone_widget import BoneWidget
...
class BVH_Joint(Joint):
    ...
    def add_child(self, child: Transform) -> None:
        if isinstance(child, BVH_Joint):
            c_x, c_y, c_z = child._local_transform[:-1, -1]
            height = np.linalg.norm([c_x, c_y, c_z])
            self.add_child(BoneWidget(height=height, base=height**.4, starting_orientation=Vectors([c_x, c_y, c_z]), name=f'{child.name}_bonewidget'))
...
class BVH(Transform, TimeManager):
    ...
    def __init__(self,
        ...
        self.root_joint: BVH_Joint
        self.name_joint_dict: Dict[str, BVH_Joint]
        self.parent_children_dict: Dict[str, List[str]]
        self.joint_num: int
        self._initialize_joint_chain(root_joint)

        self.cur_frame = 0  # initialize skeleton pose to first frame
        self.apply_frame(self.cur_frame)

    def _initialize_joint_chain(self, root_joint: BVH_Joint) -> None:
        """ Takes in BVH root joint, creates and caches data structures for easy lookup later. """
        # easy access to root_joint
        self.root_joint = root_joint
        self.add_child(self.root_joint)

        # total number of joints in skeleton
        self.joint_num = self.root_joint.joint_count()

        # quickly get the BVH_Joint from it's name
        self.name_joint_dict = {}
        for name in self.root_joint.get_chain_joint_names():
            joint_transform = self.root_joint.get_transform_by_name(name)
            assert isinstance(joint_transform, BVH_Joint)
            self.name_joint_dict[name] = joint_transform

        # quickly get the name of a BVH_Joint's children from it's own name
        self.parent_children_dict = {}
        for parent_name, bvh_joint in self.name_joint_dict.items():
            self.parent_children_dict[parent_name] = [c.name for c in bvh_joint.get_children() if isinstance(c, BVH_Joint)]
    ...
    def update(self) -> None:
        ...
        self.apply_frame(cur_frame)  # TODO adjust so this works for both streaming and non-streaming data sources

    def apply_frame(self, frame_num: int) -> None:
        """ Apply root position and joint rotation data for specified frame_num """
        self.apply_frame_root_pos(frame_num)
        self.apply_frame_rotations(frame_num)

    def apply_frame_root_pos(self, frame_num: int):
        """ Update skeleton's root to be xyz value given by pos_data """
        pos_data = self.pos_data[frame_num]
        self.root_joint.set_position(pos_data)

    def apply_frame_rotations(self, frame_num: int):
        rot_data = self.rot_data[frame_num]
        self._apply_rotations(self.root_joint, rot_data, ptr=np.array(0))

    def _apply_rotations(self, joint: BVH_Joint, rot_data: npt.NDArray[np.float32], ptr: npt.NDArray[np.int32]):
        q = Quaternions(rot_data[ptr])
        ...
        for c in joint.get_children():
            if not isinstance(c, BVH_Joint):
                continue
            self._apply_rotations(c, rot_data, ptr)
    ...
    def from_file(cls, bvh_fn: str, start_frame_idx: int = 0, end_frame_idx: Optional[int] = None) -> BVH:
        ...
        if lines.pop(0) != 'HIERARCHY':
            raise ValueError(f'Malformed BVH in line preceeding {lines}')
        ...
        # Parse motion data
        frames = np.array([list(map(float, line.strip().split(' '))) for line in lines])
        if frames.shape[0] != frame_max_num:
            raise ValueError(f'framenum specified ({frame_max_num}) and found ({frames.shape[0]}) do not match')
        ...
        if frame_max_num < end_frame_idx:
            raise ValueError(f'config specified end_frame_idx > bvh frame_max_num ({end_frame_idx} > {frame_max_num}). Replacing with frame_max_num.')
    ...
    @classmethod
    def _process_frame_data(cls, skeleton: BVH_Joint, frames: npt.NDArray[np.float32]) -> Tuple[npt.NDArray[np.float32], npt.NDArray[np.float32]]:
