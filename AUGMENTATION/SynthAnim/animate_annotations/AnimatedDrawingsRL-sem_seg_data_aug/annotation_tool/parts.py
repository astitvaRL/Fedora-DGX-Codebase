...
class InternalPart(BasePart):
    ...
    def add_sam_point(self, sam_point: SAMPoint):
        self.sam_points.append(sam_point)
    ...
    def preprocess_for_review(all_parts: List[InternalPart]):
        ...
        InternalPart.infill_parts(root_part)

    @staticmethod
    def infill_parts(part, recurse=True):

        # do this for all parts recursively
        if recurse:
            for child_part in part.children_parts:
                InternalPart.infill_parts(child_part)

        # use the full image and the mask of pixels that need to be infilled for this particular part
        l, t, r, b = part.bounding_box
        pixels_to_infill = part.part_to_be_infilled
        image = part.full_image.copy()
        infill_img = InternalPart.infiller.infill(prompt="blank", image_np=part.full_image, mask_np=pixels_to_infill)

        # overlay infill onto original image
        image[pixels_to_infill == True] = np.array(infill_img)[:, :, :3][pixels_to_infill == True]

        # crop it to get part's texture, fill in rgb values only
        part.texture_rgba[:, :, :3] = image[t:b, l:r]
