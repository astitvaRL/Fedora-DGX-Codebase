...
if __name__ == '__main__':
    ...
    # define metrics
    MeanIoU = segmentation.MeanIoU(num_classes=NUM_CLASSES, include_background=False, per_class=True, input_format='index')
    GeneralizedDiceScore = segmentation.GeneralizedDiceScore(num_classes=NUM_CLASSES, include_background=False, per_class=True, input_format='index')
    ...
    exp_eval_results_dir = "/mnt/users_scratch/astitva/WORKSPACE/Fedora-DGX-Codebase/SEGMENTATION/SAM_finetuning/EVALUATION/eval_16k_E2E_FULL/BEST_LATEST/best_eval/"
    # exp_eval_results_dir = '/mnt/users_scratch/astitva/WORKSPACE/Fedora-DGX-Codebase/SEGMENTATION/SAM_finetuning/EVALUATION/eval_data_split_12k/best/best_eval/'
    # exp_eval_results_dir = "/mnt/users_scratch/astitva/WORKSPACE/Fedora-DGX-Codebase/SEGMENTATION/SAM_finetuning/EVALUATION/16k_ANIMSEG_DecoderOnly_ALL_CLASSES/LATEST/best_eval/"
    # exp_eval_results_dir = "/mnt/users_scratch/astitva/WORKSPACE/Fedora-DGX-Codebase/SEGMENTATION/SAM_finetuning/EVALUATION/16k_ANIMSEG_E2E_ALL_CLASSES_SlowLR/best/best_eval/"
    sapiens_eval_results_dir = "/mnt/users_scratch/astitva/DATA/LIP_drawings_16k/sapiens_val_prediction_400/sapiens_0.3b/"
    
    ref_dir = join(data_root, label_id_dir_name)
    files = sorted(os.listdir(ref_dir))[-2000:]

    classwise_ious = torch.zeros((NUM_CLASSES-1,))
    classwise_gds = torch.zeros((NUM_CLASSES-1,))
    ...
    for filename in tqdm(files):
        if filename.endswith('.png'):
            ...
            # exp_seg = cv2.imread(join(exp_eval_results_dir, f'{filename}/pred.png'))
            exp_seg = np.load(join(sapiens_eval_results_dir, f'{filename[:-4]}_seg.npy'))
            gt_seg = cv2.imread(join(exp_eval_results_dir, f'{filename_base}/gt_all.png'))
            ...
            exp_t = torch.Tensor(exp_seg).long().unsqueeze(0)
    ...
    final_mean_iou = avg_classwise_ious.sum()/(NUM_CLASSES-1)
    final_mean_gds = avg_classwise_gds.sum()/(NUM_CLASSES-1)
    final_mean_acc = avg_classwise_acc[1:].sum()/(NUM_CLASSES-1)
