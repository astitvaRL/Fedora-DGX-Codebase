...
def build_sam_vit_b_infer(num_classes, checkpoint=None):
    return _build_sam(
        encoder_embed_dim=768,
        encoder_depth=12,
        encoder_num_heads=12,
...
def _build_sam(
    ...
    if checkpoint is not None:
        ...
        try:
            ...
            if load_mode=='post_training':
                sam.load_state_dict(new_state_dict)
            else:
                encoder_state_dict = {k: v for k, v in new_state_dict.items() if not 'prompt_encoder' and not 'mask_decoder' in k}
                sam.load_state_dict(encoder_state_dict, strict=False)
    return sam
