...
if __name__ == '__main__':
    ...
    # create dataloader
    train_dataloader = DataLoader(train_dataset, batch_size=32, num_workers=0, shuffle=True, drop_last=True, collate_fn=collate_fn)
    test_dataloader = DataLoader(test_dataset, batch_size=32, num_workers=0, shuffle=False, drop_last=True, collate_fn=collate_fn)
    ...
    optimizer = torch.optim.Adam(params, lr=1e-5, weight_decay=0) # adding all parameters to optimizer as an iterable
    ...
    for epoch in range(epoch_start, num_epochs):
        ...
        for step, (images, targets) in enumerate(tqdm(train_dataloader, "Training")):
            ...
            losses = sum(loss for loss in loss_dict.values())
                # mask_predictions = mask_rcnn(images, targets)
