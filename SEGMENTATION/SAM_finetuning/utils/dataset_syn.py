...
class Dataset_body(Dataset): 
    def __init__(self, sam_model, data_root, labels_definition_file_path, img_dir_name, img_embed_dir_name, label_id_dir_name, mode='train', device='cuda', return_embeddings=False):
        ...
        self.files = sorted(os.listdir(join(self.data_root, self.img_embed_dir_name)))[:-self.num_test_samples]
        if self.mode == 'test':
            self.files = sorted(os.listdir(join(self.data_root, self.img_embed_dir_name)))[-self.num_test_samples:]
    ...
    def __getitem__(self, index):
        ...
        if not self.cache_available: 
            ...
            image = cv2.imread(join(self.data_root, self.image_dir_name, self.files[index][:-4]+'.png'))
        ...
        if not self.cache_available: 
            # populate cache entry during first-time access
            gt2D = cv2.imread(join(self.data_root, self.label_id_dir_name, self.files[index][:-6]+'_1024.png'))
...
# dataset definition for only body (no facial details) synthetic data
class Dataset_body_ternary(Dataset): 
    def __init__(self, sam_model, data_root, labels_definition_file_path, img_dir_name, img_embed_dir_name, label_id_dir_name, mode='train', device='cuda', return_embeddings=False):
        self.sam_model = sam_model
        self.num_classes = 3
...
class Dataset_body_real(Dataset): 
    def __init__(self, sam_model, data_root, labels_definition_file_path, img_dir_name, img_embed_dir_name, label_id_dir_name, mode='train', device='cuda', return_embeddings=False):
        self.sam_model = sam_model
        self.num_classes = 18
        self.semantics = SemanticSegmentation(labels_definition_path=labels_definition_file_path, num_classes=self.num_classes)
