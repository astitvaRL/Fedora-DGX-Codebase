...
if __name__ == '__main__':
    ...
    image_dir_name = 'MANIFOLD/animated_drawings_images_prior_april22/unlabelled_images/'
    # image_dir_name = 'MANIFOLD/Stimuli_HighRes'
    label_id_dir_name = 'AD_SegMaps/labels_7k_1024' 
    cache_dir = join(data_root, 'dataset_caches/cache_UNLABELLED')
    ...
    out_dir = 'inference_unlabelled_images'
    mode = 'inference'
    ...
    cache_path = join(cache_dir, f'{mode}_cache_{max_num_samples}.pt')
    ...
    # create dataset
    inference_dataset = DrawingsDatasetInference(sam_model, labels_definition_file_path=labels_definition_file_path, data_root = data_root, img_dir_name=image_dir_name, max_num_samples=max_num_samples)
    ...
    inference_dataset.semantics_coarse = semantics_coarse
    inference_dataset.semantics_fine = semantics_fine
    ...
    if cache_available:
        ...
        inference_dataset.load_cache(cache_path)
    else:
    ...
    # create dataloader
    inference_dataloader = DataLoader(inference_dataset, batch_size=16, num_workers=0, shuffle=False, drop_last=True)

    # define differntiable non-learnable upsampling layer
    upsample = torch.nn.Upsample(scale_factor=4, mode='nearest')

    if not load_best_eval_ckpt:
        print(f'INFER at Epoch-{epoch}')
    else:
        print(f'INFER at Best-Eval Epoch')
    ...
    for step, image_data_infer, in enumerate(tqdm(inference_dataloader,"EVAL")):
        ...
        with torch.no_grad():
            ...
            embedding_coarse = image_encoder_coarse(image_data_infer)
            ...
            for batch_idx in range(image_data_infer.shape[0]):
                ...
                ax[3].set_title("Overlayed", fontsize=TITLE_SIZE)
                ax[3].axis('off')
                if refine_masks:
...
if not(cache_available):
    inference_dataset.save_cache(cache_path)
