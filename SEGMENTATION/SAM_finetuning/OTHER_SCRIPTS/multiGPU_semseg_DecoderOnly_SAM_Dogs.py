...
if __name__ == '__main__':
    ...
    train_label_id_dir_name = 'train_segmentations' 
    test_image_dir_name = 'val_images'
    test_label_id_dir_name = 'val_segmentations' 
    cache_dir = join(data_root, 'dataset_caches/cache_DOGS')
    os.makedirs(cache_dir, exist_ok=True)
    train_cache_path = join(cache_dir, 'train_cache.pt')
    test_cache_path = join(cache_dir, 'test_cache.pt')
    task_name = 'ANIMSEG_DecoderOnly_SAMEncoder_DOGS' # finetuned checkpoint will be saved here
    ...
    num_classes = 8 # 7 + BG
    # we will use 8 arbitrary classes from 18 classes of drawings for visualization
    semantics = SemanticSegmentationNoFace(labels_definition_file_path, num_classes=18)
    ...
    # create dataset
    train_dataset = DogsDataset(sam_model, labels_definition_file_path=labels_definition_file_path, data_root = data_root, img_dir_name=train_image_dir_name, label_id_dir_name = train_label_id_dir_name, mode='train')
    test_dataset = DogsDataset(sam_model, labels_definition_file_path=labels_definition_file_path, data_root = data_root, img_dir_name=test_image_dir_name, label_id_dir_name = test_label_id_dir_name, mode='test')
    ...
    all_params = list(sam_model.prompt_encoder.parameters()) + list(sam_model.mask_decoder.parameters())
    ...
    for epoch in range(epoch_start, num_epochs):
        ...
        for step, (image_data_cpu, gt, bg_mask, bbox) in enumerate(tqdm(train_dataloader, "Training")):
            ...
            # not computing gradients for image encoder
            with torch.no_grad():
            ...
            # computing gradients for mask decoder
