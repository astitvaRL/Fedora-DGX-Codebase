...
if __name__ == '__main__':
    ...
    task_name = 'CartoonDogs_pretrainedSAM_All14k_SaveEval' # finetuned checkpoint will be saved here
    ...
    if resume_training:
        ...
        resume_ckpt = "/mnt/users_scratch/astitva/WORKSPACE/Fedora-DGX-Codebase/SEGMENTATION/SAM_finetuning/checkpoints/OLD_CKPTS/ANIMSEG_CoarsePoints_E2E_7k_corrupted/model_eval_best.pth"
    ...
    num_classes = 8
    semantics = SemanticSegmentationAll(labels_definition_file_path, num_classes=27) # for selecting arbitrary colors
        ...
        if epoch%eval_frequency==0:    
            ...
            for step, (image_data_eval, gt, bg_mask, bbox) in enumerate(tqdm(test_dataloader,"EVAL")):
                ...
                with torch.no_grad():
                    ...
                    image_data_vis = np.transpose(image_data_vis,(1,2,0))
                    cv2.imwrite(f"{eval_epoch_dir}/{step}_id.png",labels_out.cpu().numpy().astype('uint8'))
                    cv2.imwrite(f"{eval_epoch_dir}/{step}_seg.png",labels_out_vis)
                    # plot eval results
                    TITLE_SIZE = 30
                    fig, ax = plt.subplots(1,4, figsize=(40,10))
