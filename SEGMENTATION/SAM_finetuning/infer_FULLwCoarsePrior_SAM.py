...
if __name__ == '__main__':
    ...
    for step, (image_name_string, image_data_eval, gt2D_coarse) in enumerate(tqdm(test_dataloader,"EVAL")):
        valid_face_detected = False # reset flag for each image
        image_data_eval = image_data_eval.to(device)
        gt2D_coarse = gt2D_coarse.to(device)
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(image_data_eval.shape[0]):
                ...
                if visualize_coarse:
                    ...
                    ax[1].set_title("Input Coarse Prior", fontsize=TITLE_SIZE)
                    ax[1].axis('off')
