...
if __name__ == '__main__':
    ...
    cache_dir = join(data_root, 'dataset_caches/cache_FACE_REAL16k')
    os.makedirs(cache_dir, exist_ok=True)
    train_cache_path = join(cache_dir, 'train_cache.pt')
    test_cache_path = join(cache_dir, 'test_cache.pt')
    task_name = '16k_ANIMSEG_E2E_FACE_wBinMask' # finetuned checkpoint will be saved here
    ...
    # create dataset
    train_dataset = DrawingsDatasetFace(sam_model, labels_definition_file_path=labels_definition_file_path, data_root = data_root, img_dir_name=image_dir_name, label_id_dir_name = label_id_dir_name, mode='train', num_test_samples=2000)
    test_dataset = DrawingsDatasetFace(sam_model, labels_definition_file_path=labels_definition_file_path, data_root = data_root, img_dir_name=image_dir_name, label_id_dir_name = label_id_dir_name, mode='test', num_test_samples=2000)
