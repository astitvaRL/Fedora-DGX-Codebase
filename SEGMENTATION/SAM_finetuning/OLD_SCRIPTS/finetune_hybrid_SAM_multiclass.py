...
from utils.dataset import Dataset_hybrid
...
if __name__ == '__main__':
    ...
    task_name = 'animseg_synth_3k_hybrid' # finetuned checkpoint will be saved here
    ...
    # create dataset
    train_dataset = Dataset_hybrid(sam_model, data_root = data_root, img_dir_name=image_dir_name, img_embed_dir_name = embed_dir_name, label_id_dir_name = label_id_dir_name, mode='train')
    test_dataset = Dataset_hybrid(sam_model, data_root = data_root, img_dir_name=image_dir_name, img_embed_dir_name = embed_dir_name, label_id_dir_name = label_id_dir_name, mode='test')
   ...
   # Freeze all layers of image encoder
    ...
    # set requires_grad=True only for the neck layer
    ...
    for epoch in range(num_epochs):
        epoch_loss = 0
        # TRAINING
        for step, (image_data, image_embedding, gt, binmask, bbox) in enumerate(tqdm(train_dataloader)):
            ...
            # add fixed and learnable encoder embeddings
            new_embedding = sam_model.image_encoder(image_data)
            embedding_hybrid =  new_embedding + image_embedding.to(device)

            mask_predictions, _ = sam_model.mask_decoder(
                image_embeddings=embedding_hybrid, # (B, 256, 64, 64)
        ...
        if epoch%eval_frequency==0:    
            ...
            eval_loss = 0
            for step, (image_data, image_embedding, gt, binmask, bbox) in enumerate(test_dataloader):
