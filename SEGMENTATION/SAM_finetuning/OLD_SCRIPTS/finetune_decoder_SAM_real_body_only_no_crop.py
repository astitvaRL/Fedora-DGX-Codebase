...
from utils.dataset import Dataset_body, Dataset_precomputed_body, Dataset_precomputed_body_real
...
if __name__ == '__main__':
    ...
    image_dir_name = 'amateur_drawings'
    ...
    task_name = 'no_crop_2k_real' # finetuned checkpoint will be saved here
    ...
    if precompute_embeddings:
        ...
        for name in tqdm(names):
            ...
            image_data_path = join(data_root, image_dir_name, name[0], name)
            embedding_save_path = join(embedding_dir_path, name.split('.png')[0]+'.npy')
            if os.path.exists(embedding_save_path):
                continue
            image_data = io.imread(image_data_path)
    ...
    # create dataset
    train_dataset = Dataset_precomputed_body_real(labels_definition_file_path=labels_definition_file_path, data_root = data_root, img_dir_name=image_dir_name, img_embed_dir_name = embed_dir_name, label_id_dir_name = label_id_dir_name, mode='train')
    test_dataset = Dataset_precomputed_body_real(labels_definition_file_path=labels_definition_file_path, data_root = data_root, img_dir_name=image_dir_name, img_embed_dir_name = embed_dir_name, label_id_dir_name = label_id_dir_name, mode='test')
