...
if __name__ == '__main__':
    ...
    SAM_EVAL_ROOT = "/mnt/users_scratch/astitva/WORKSPACE/Fedora-DGX-Codebase/SEGMENTATION/SAM_finetuning/INFERENCE/OOD_paper_data_splits/"
    exp1_out_dir = join(SAM_EVAL_ROOT,"infer0.5k_ANIMSEG_E2E_FINE/best_eval_epoch/semantics")
    exp2_out_dir = join(SAM_EVAL_ROOT,"infer1k_ANIMSEG_E2E_FINE/best_eval_epoch/semantics")
    exp3_out_dir = join(SAM_EVAL_ROOT,"infer2k_ANIMSEG_E2E_FINE/best_eval_epoch/semantics")
    exp4_out_dir = join(SAM_EVAL_ROOT,"infer4k_ANIMSEG_E2E_FINE/best_eval_epoch/semantics")
    exp5_out_dir = join(SAM_EVAL_ROOT,"infer8k_ANIMSEG_E2E_FINE/best_eval_epoch/semantics")
    exp6_out_dir = join(SAM_EVAL_ROOT,"infer10k_ANIMSEG_E2E_FINE/best_eval_epoch/semantics")
    exp7_out_dir = join(SAM_EVAL_ROOT,"infer12k_ANIMSEG_E2E_FINE/best_eval_epoch/semantics")
    exp8_out_dir = join(SAM_EVAL_ROOT,"infer16k_ANIMSEG_E2E_FINE/best_eval_epoch/semantics")

    OUT_DIR = './COMPARISON/DATA_SPLITS_ALL_OOD/'
    ...
    for filename in tqdm(files):
        if True:
            input_im = cv2.imread(join(exp1_out_dir, f'{filename}/{filename}_original.png'))
            exp1_seg = cv2.imread(join(exp1_out_dir, f'{filename}/{filename}_seg_fine_masked.png'))
            exp2_seg = cv2.imread(join(exp2_out_dir, f'{filename}/{filename}_seg_fine_masked.png'))
            exp3_seg = cv2.imread(join(exp3_out_dir, f'{filename}/{filename}_seg_fine_masked.png'))
            exp4_seg = cv2.imread(join(exp4_out_dir, f'{filename}/{filename}_seg_fine_masked.png'))
            exp5_seg = cv2.imread(join(exp5_out_dir, f'{filename}/{filename}_seg_fine_masked.png'))
            exp6_seg = cv2.imread(join(exp6_out_dir, f'{filename}/{filename}_seg_fine_masked.png'))
            exp7_seg = cv2.imread(join(exp7_out_dir, f'{filename}/{filename}_seg_fine_masked.png'))
            exp8_seg = cv2.imread(join(exp8_out_dir, f'{filename}/{filename}_seg_fine_masked.png'))
            ...
            if plot:
                # plot
                TITLE_SIZE = 30
                fig, ax = plt.subplots(1,9, figsize=(180,20))
                fig.tight_layout()
