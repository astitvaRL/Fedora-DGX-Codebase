...
from utils.dataset_dogs import DogsDatasetEval
...
if __name__ == '__main__':
    ...
    for step, (image_data_eval, gt, image_name_string) in enumerate(tqdm(test_dataloader,"EVAL")):
        ...
        with torch.no_grad():
            ...
            for class_idx in range(1, mask_predictions.shape[1]):
                batch_IoU = monai.metrics.compute_iou(pred_one_hot[:,class_idx,:,:].unsqueeze(1), gt[:,class_idx,:,:].unsqueeze(1), include_background=True, ignore_empty=False)
            ...
            for batch_idx in range(image_data_eval.shape[0]):
                ...
                # Ground Truth
                # all
                gt_labels = torch.argmax(torch.Tensor(gt[batch_idx]), dim=0)  # last sample from batch
                gt_vis = semantics.labels_to_colors(gt_labels.cpu().numpy().astype('uint8'))
